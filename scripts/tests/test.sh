...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -t --test-case [arg] 	Test case(s) to run. Can be repeated.
  -l --list             List all test cases
  -c --describe  [arg]  Describe a test case.
  -D --use-defaults     Use default parameters for test cases. Otherwise, test cases will run in interactive mode (ask you for the parameters)
...
if [[ -n "${arg_c}" ]] ; then
	TEST_CASE="${__dir}/cases/${arg_c%%.sh}.sh"
...
if [[ -n "${arg_t:-}" ]] && declare -p arg_t 2> /dev/null | grep -q '^declare \-a'; then
	...
	error "Must provide at least one test case"
	help
fi
